import{a as m,i as b}from"./vendor-ce9f25a7.js";document.addEventListener("DOMContentLoaded",function(){var e=window.location.href,s=document.getElementById("homeLink"),t=document.getElementById("favoritesLink");e.includes("index.html")?(s.classList.add("active"),t.classList.remove("active")):e.includes("favourite.html")&&(t.classList.add("active"),s.classList.remove("active"))});const k=document.querySelector(".open-mobile-menu-btn"),E=document.querySelector(".close-mobile-menu-btn"),S=document.querySelector(".mobile-menu-wrapper");k.addEventListener("click",function(){S.classList.add("is-open"),document.body.classList.add("not-scrollable")});E.addEventListener("click",function(){S.classList.remove("is-open"),document.body.classList.remove("not-scrollable")});const q="https://energyflow.b.goit.study/api";m.defaults.baseURL=q;async function J(){return(await m.get("/quote")).data}async function z(e){return(await m.post("/subscription",{email:e})).data}async function B(e,{email:s,rate:t,comment:a}){return await m.patch(`/exercises/${e}/rating`,{rate:t,email:s,review:a})}const $=document.getElementById("form-close-btn"),u=document.querySelector(".backdrop"),I=document.querySelector("#user-email"),C=document.getElementById("user-comment"),A=document.querySelector(".form-send-btn");$.addEventListener("click",()=>{u.classList.remove("is-open")});u.addEventListener("click",()=>{u.classList.remove("is-open")});const F=document.querySelector(".rating-wrapper"),h=document.querySelector(".rating-star-value"),n={rate:0,email:"",comment:""};F.addEventListener("click",e=>{const s=document.querySelectorAll(".rating-star-icons");n.rate=e.target.dataset.id;for(let t=0;t<5;t++)t<n.rate?(s[t].style.fill="#EEA10C",h.textContent=n.rate===void 0?"0.0":`${n.rate}.0`):(s[t].style.fill="rgba(27, 27, 27, 0.20)",h.textContent=n.rate===void 0?"0.0":`${n.rate}.0`)});let w=null;const j=document.querySelector(".backdrop-form");function M(e){w=e,u.classList.add("is-open")}j.addEventListener("submit",T);async function T(e){if(e.preventDefault(),n.email=I.value.trim(),n.comment=C.value.trim(),n.email!==""&&n.rate!==0){A.disabled=!1;try{const s=await B(w,n);b.success({message:"Your rating is accepted",position:"topRight",color:"green"}),u.classList.remove("is-open")}catch(s){b.error({message:`${s.message}`,position:"topRight",color:"red"}),u.classList.remove("is-open")}}}const o="favourite",O=e=>{localStorage.setItem(o,JSON.stringify(e))},f=e=>{try{return JSON.parse(localStorage.getItem(e))}catch(s){console.log(s.message)}},x=e=>{const s=f(o).filter(t=>t._id!==e);localStorage.removeItem(o),localStorage.setItem(o,JSON.stringify(s))},v=document.querySelector(".exr-card-backdrop");let i=!1,d=[],L=JSON.parse(localStorage.getItem("favourite"));L&&L.forEach(e=>{d[0]||(d[0]=e),d.push(e)});function p(e){return`${e.charAt(0).toUpperCase()}${e.slice(1)}`}function R(e,s=!1,t=!1){i=s,i||d.forEach(a=>{a._id===e._id&&(i=!0)}),N(e),v.classList.add("card-is-open"),document.body.classList.add("not-scrollable"),i===!0&&(document.querySelector(".add-favourite-btn").innerHTML=`Remove from
    ...
    <div>
      ...
      <div class="buttons-cont">
    ...
    </div>`;v.innerHTML=t;const a=document.querySelectorAll(".star-rating-icon");for(let c=0;c<Math.round(e.rating);++c)a[c].style.fill="#eea10c";const r=document.querySelector(".add-favourite-btn");r.addEventListener("click",function(){i?(x(e._id),r.innerHTML=`Add to favourite
          <svg class="heart-icon">
            <use href="/energy_flow_project_06/assets/icon-c8fc18a4.svg#icon-heart"></use>
          </svg>`,i=!1):(d.push(e),O(d),r.innerHTML=`Remove from
          <svg class="heart-icon">
            <use href="/energy_flow_project_06/assets/icon-c8fc18a4.svg#icon-heart"></use>
          </svg>`,i=!0),y()}),document.getElementById("close-card").addEventListener("click",()=>{v.classList.remove("card-is-open"),document.body.classList.remove("not-scrollable")}),document.querySelector(".exr-card-backdrop").addEventListener("click",()=>{v.classList.remove("card-is-open"),document.body.classList.remove("not-scrollable")}),document.querySelector(".give-rating-btn").addEventListener("click",()=>{v.classList.remove("card-is-open"),document.body.classList.remove("not-scrollable"),M(e._id)})}const l={cardSet:document.querySelector(".fav_card_list"),noCards:document.querySelector(".no_cards_wrapper")},P=e=>{const s=new Set;return e.filter(a=>s.has(a._id)?!1:(s.add(a._id),!0))},H=e=>{const t=P(e).map(({name:a,_id:r,burnedCalories:c,bodyPart:g,target:_})=>`<li data-id-card="${r}" data-component="fav_card" class="list_item">
          <div class="fav_card">
        ...
        </li>`);l.cardSet.innerHTML=t.join("")},U=e=>{const s=e.target.closest('[data-action="start_exercise_btn"]'),t=e.target.closest('[data-action="delete_fav_card"]'),a=e.target.closest('[data-component="fav_card"]');if(!(!e.target.tagName==="BUTTON"||!t&&!s)){if(t)if(t.dataset.idDelBtn===a.dataset.idCard){const r=t.dataset.idDelBtn;x(r),y()}else return;else if(s){const r=s.dataset.idStartBtn,g=f(o).find(_=>_._id===r);R(g,!0,!0)}}},y=()=>{document.querySelector(".favourite_exercises")!==null&&(f(o)!==null?(l.noCards.classList.add("visually-hidden"),l.cardSet.classList.remove("visually-hidden"),H(f(o)),l.cardSet.addEventListener("click",U)):(l.noCards.classList.remove("visually-hidden"),l.cardSet.classList.add("visually-hidden")))};y();export{J as g,R as h,z as p};
//# sourceMappingURL=favourite_exercises-6d225b43.js.map
